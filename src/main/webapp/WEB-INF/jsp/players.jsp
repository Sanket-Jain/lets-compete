...
<div class="page">
  ...
  <div class="card">
    <div class="table-wrap">
      <table>
        <thead>
          <tr><th>#</th><th>Name</th><th>Skill Level</th><th>Achievements</th><th>Score</th><th>W / P</th><th>Actions</th></tr>
        </thead>
...
<div class="modal-overlay" id="addPlayerModal">
  <div class="modal">
    ...
    <div class="form-row"><div class="form-group"><label>Skill Level</label><select id="p-skill"><option value="BEGINNER">Beginner</option><option value="INTERMEDIATE">Intermediate</option><option value="PRO">Pro</option></select></div></div>
...
<div class="modal-overlay" id="editPlayerModal">
  <div class="modal">
    ...
    <div class="form-row"><div class="form-group"><label>Skill Level</label><select id="ep-skill"><option value="BEGINNER">Beginner</option><option value="INTERMEDIATE">Intermediate</option><option value="PRO">Pro</option></select></div></div>
...
<script>
  const skillBadge = s => ({ PRO:'<span class="badge badge-pro">PRO</span>', INTERMEDIATE:'<span class="badge badge-intermediate">INTERMEDIATE</span>', BEGINNER:'<span class="badge badge-beginner">BEGINNER</span>' })[s] || s;
  ...
  async function loadPlayers() {
    try {
      ...
      if (!ps.length) { tb.innerHTML='<tr><td colspan="7"><div class="empty"><div class="empty-icon">👤</div><p>No players yet.</p></div></td></tr>'; return; }
      tb.innerHTML = ps.map((p,i)=>`<tr>
        <td class="text-muted">${i+1}</td>
        <td><strong>${p.name}</strong></td>
        <td>${skillBadge(p.skillLevel)}</td>
        <td class="text-muted" style="max-width:220px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${p.achievements||'—'}</td>
  ...
  async function savePlayer(){
    ...
    try{
      await api('POST','/api/players',{name,skillLevel:document.getElementById('p-skill').value,achievements:document.getElementById('p-ach').value});
  ...
  function editPlayer(id,name,skill,ach){
    ...
    document.getElementById('ep-skill').value=skill; document.getElementById('ep-ach').value=ach;
  ...
  async function updatePlayer(){
    ...
    if(!name){showToast('Name required','error');return;}
    try{
      await api('PUT',`/api/players/${id}`,{name,skillLevel:document.getElementById('ep-skill').value,achievements:document.getElementById('ep-ach').value});
